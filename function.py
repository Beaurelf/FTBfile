...
def icon(p):
    if (os.path.splitext(p))[1] == ".docx" or (os.path.splitext(p))[1] == ".doc" or (os.path.splitext(p))[1] == ".DOCX"or (os.path.splitext(p))[1] == ".DOC":
        img = PhotoImage(file='images/Word_2013_23473 (1).png')
        return img
    if (os.path.splitext(p))[1] == ".pdf" or (os.path.splitext(p))[1] == ".PDF":
        img = PhotoImage(file='images/adobe_pdf_document_14979 (1).png')
        return img
    ...
    if (os.path.splitext(p))[1] == ".mp4" or (os.path.splitext(p))[1] == ".mkv" or (os.path.splitext(p))[1] == ".avi" \
        ...
        img = PhotoImage(file='images/Videos_31092 (1).png')
        return img
    if (os.path.splitext(p))[1] == ".xlsx":
        img = PhotoImage(file='images/Excel_2013_23480 (1).png')
        return img
    ...
    if (os.path.splitext(p))[1] == ".lnk" or (os.path.splitext(p))[1] == ".LNK" :
        img = PhotoImage(file='images/shortcut_23045 (1).png')
        return img
    if (os.path.splitext(p))[1] == ".txt":
        img = PhotoImage(file='images/Text_edit_35243 (1).png')
        return img
    if (os.path.splitext(p))[1] == ".mp3" or (os.path.splitext(p))[1] == ".m4a":
        img = PhotoImage(file='images/iconfinder-musicmelodysoundaudio36-4105545_113821 (1).png')
        return img
    if (os.path.splitext(p))[1] == ".iso":
        img = PhotoImage(file='images/61.png')
        return img
    ...
    if (os.path.splitext(p))[1] == ".exe":
        img = PhotoImage(file='images/Programs (1).png')
        return img
    if (os.path.splitext(p))[1] == ".pptx" or (os.path.splitext(p))[1] == ".ppt":
        img = PhotoImage(file='images/PowerPoint_2013_23479 (1).png')
        return img
    if (os.path.splitext(p))[1] == ".pub":
        img = PhotoImage(file='images/Publisher_2013_23475 (1).png')
        return img
    if (os.path.splitext(p))[1] == ".dll":
        img = PhotoImage(file='images/dll (1).png')
        return img
    if (os.path.splitext(p))[1] == ".torrent":
        img = PhotoImage(file='images/torrent_icon-icons.com_72051 (1).png')
        return img
    else:
        img = PhotoImage(file='images/Blank (1).png')
        return img
...
def nb_file(path):
    ...
    try:
        for x in os.listdir(path):
            path1 = Path(os.path.join(path, x))
            if path1.is_file():
                i = i + 1
            else:
                i = i + nb_file(path1)
